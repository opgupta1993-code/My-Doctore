...
class NavigationController extends GetxController {
  ...
  Future<void> initializeSdk() async {
    ...
    try {
      ...
      try {
        Response? response;
        response = await sdkWebViewController.getApiResponse(flowType, response, presenter);
  ...
  getInAppWebViewInstance(BuildContext context) {
    ...
    return[ InAppWebView(
      ...
      onCloseWindow: (controller) async{
        await inAppWebViewController.evaluateJavascript(source: "window.localStorage.clear();");
      },
  ...
  Future<bool?> setChildWindow(InAppWebViewController controller, CreateWindowAction createWindowRequest) async {
  ...
  void _updateParams(InAppWebViewController controller, Uri? uri) {
    ...
    if (redirectUrl != null && uri.toString().startsWith(redirectUrl)) {
      ...
      presenter.sdkContext?.scope.set("final_response.isCancelledByUser", false);
  ...
  void _loadWebPage(InAppWebViewController controller, Uri? uri) async {
    ...
    if(uri.toString().contains("billdesksdk://web-flow")){
      ...
      controller.evaluateJavascript(source: """
        ...
        presenter.sdkContext?.scope.set("final_response.isCancelledByUser", _getSdkState(params["status"]!));
        presenter.sdkContext?.scope.set("bd-modal.shouldModalClose", true);
        bdModelShouldModalClose.value = true;
        isDone.value  = false;
  ...
  Future<ServerTrustAuthResponse?> setCertificateToSite(
    ...
    if(!url.contains("${BuildConfig.filePath}") && !urls.contains(url) && isValidError) {
    ...
    dialogResponse = await showDialog<ServerTrustAuthResponseAction>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Ssl Certificate Error: $sslErrorMessage!\n Url : $url'),
          actions: <Widget>[
            TextButton(
              child: Text('Cancel'),
              onPressed: () {
                Navigator.of(context).pop(ServerTrustAuthResponseAction.CANCEL);
              },
            ),
            TextButton(
              child: Text('Proceed'),
              onPressed: () {
                Navigator.of(context).pop(ServerTrustAuthResponseAction.PROCEED);
              },
            ),
          ],
        );
    ...
    if(dialogResponse == ServerTrustAuthResponseAction.CANCEL){
        ...
        controller.clearCache();
        sdkWebViewController.exitAndInvokeCallback(false, presenter, context, isSSLError: true);

    }
    // Return the appropriate action based on the user's choice
    return ServerTrustAuthResponse(action: dialogResponse ?? ServerTrustAuthResponseAction.PROCEED);
